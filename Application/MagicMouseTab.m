...
- (void)awakeFromNib {
    ...
    [rdMMHanded selectCellAtRow:0 column:enMMHanded];
    [cbAll setState:enMMAll];
    [self enUpdated];
...
- (void)showPreview:(BOOL)scroll {
    ...
    if (saveRowIndex != rowIndex) {
        ...
        [gesturePreviewView stopTimer];
        gesturePreviewView = [[GesturePreviewView alloc] initWithDevice:1];
        [gesturePreviewView setHanded:enMMHanded];
...
- (NSInteger)outlineView:(NSOutlineView *)outlineView numberOfChildrenOfItem:(id)item {
    return (item == nil) ? [magicMouseCommands count] : [[item objectForKey:@"Gestures"] count];
}
...
- (id)outlineView:(NSOutlineView *)outlineView child:(NSInteger)index ofItem:(id)item {
    return (item == nil) ? [magicMouseCommands objectAtIndex:index] : [[item objectForKey:@"Gestures"] objectAtIndex:index];
}
...
- (void)loadActionButton {
    ...
    if ([gesture isEqualToString:@"V-Shape"]) {
        ...
        if (![[applicationButton titleOfSelectedItem] isEqualToString:@"Safari"]) {
            ...
            [builtinCommands removeObject:@"Quick Tab Switching"];
        }
    ...
    if (saveTitle == nil || [saveTitle isEqualToString:@""]) {
...
- (void)loadGestureTableView {
    ...
    if (addsCommand) {
        ...
        for (NSString *gesture in allGestures) {
            if ([[magicMouseMap objectForKey:[applicationButton titleOfSelectedItem]] objectForKey:gesture] == nil) {
...
- (IBAction)commitCommandSheet:(id)sender {
    ...
    if (!addsCommand) {
        ...
        [[magicMouseMap objectForKey:oldApp] removeObjectForKey:[oldItem objectForKey:@"Gesture"]];
        [[[commandOutlineView parentForItem:oldItem] objectForKey:@"Gestures"] removeObject:oldItem];
    }

    {
        //user adds a command
        NSUInteger i, count = [magicMouseCommands count];
        for (i = 0; i < count; i++) {
            if ([[[magicMouseCommands objectAtIndex:i] objectForKey:@"Application"] isEqualToString:newApplication]) {
                NSMutableArray *tmp = [[magicMouseCommands objectAtIndex:i] objectForKey:@"Gestures"];
                ...
                //[[[magicMouseCommands objectAtIndex:i] objectForKey:@"Gestures"] addObject:newCommand];
                [[[magicMouseCommands objectAtIndex:i] objectForKey:@"Gestures"] insertObject:newCommand atIndex:j];


                [[magicMouseMap objectForKey:newApplication] setObject:newCommand forKey:newGesture];
                break;
            }
        }
        if (i == count) {
        ...
        [commandOutlineView reloadItem:nil reloadChildren:YES];
        [commandOutlineView expandItem:[magicMouseCommands objectAtIndex:i] expandChildren:YES];
...
- (IBAction)removeCommand:(id)sender {
    ...
    if ([commandOutlineView levelForItem:item] == 0) {
        [magicMouseMap removeObjectForKey:[item objectForKey:@"Application"]];

        [magicMouseCommands removeObject:item];
    } else {
        NSString *app = [[commandOutlineView parentForItem:item] objectForKey:@"Application"];
        [[magicMouseMap objectForKey:app] removeObjectForKey:[item objectForKey:@"Gesture"]];
...
- (IBAction)change:(id)sender {
    if (sender == actionButton) {
        ...
        [self loadGestureTableView];
        [self loadActionButton];
    } else if (sender == rdMMHanded) {
        enMMHanded = (int)[sender selectedColumn];
        [Settings setKey:@"MMHanded" withInt:(int)[sender selectedColumn]];
    } else if (sender == cbAll) {
        enMMAll = [sender state] == NSControlStateValueOn ? 1: 0;
        [Settings setKey:@"enMMAll" withInt:enMMAll];
        [self enUpdated];
    }
...
- (BOOL)tableView:(NSTableView *)aTableView shouldSelectRow:(NSInteger)rowIndex {
    if (addsCommand) {
        return [[magicMouseMap objectForKey:[applicationButton titleOfSelectedItem]] objectForKey:[allGestures objectAtIndex:rowIndex]] == nil;
    } else {
        NSDictionary *tmp = [magicMouseMap objectForKey:[applicationButton titleOfSelectedItem]];
...
- (void)tableView:(NSTableView *)aTableView willDisplayCell:(id)aCell forTableColumn:(NSTableColumn *)aTableColumn row:(NSInteger)rowIndex {
    if (addsCommand) {
        if ([[magicMouseMap objectForKey:[applicationButton titleOfSelectedItem]] objectForKey:[allGestures objectAtIndex:rowIndex]] == nil)
...
- (IBAction)restoreDefaults:(id)sender {
    ...
    [alert setInformativeText:@"Your current Magic Mouse settings will be deleted."];
