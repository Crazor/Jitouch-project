...
static CFTypeRef getForemostApp() {
    ...
    if (AXUIElementCopyAttributeValue(focusedAppRef, kAXTitleAttribute, &titleRef) == kAXErrorSuccess) {
        if (
            [(__bridge NSString*)titleRef isEqualToString:@"Notification Center"] ||
            [(__bridge NSString*)titleRef isEqualToString:@"Control Center"]
        ) {
            CFRelease(titleRef);
            return NULL;
        }
...
static NSString* nameOfAxui(CFTypeRef ref) {
    ...
    return (__bridge NSString *)processName;
}
...
static NSString* commandForGesture(NSString *gesture, int device) {
    ...
    CFSafeRelease((CFStringRef)CFBridgingRetain(application));
    CFSafeRelease(axui);
...
static void doCommand(NSString *gesture, int device) {
    ...
    if (commandDict && [[commandDict objectForKey:@"Enable"] boolValue]) {
        ...
        if ([[commandDict objectForKey:@"IsAction"] boolValue]) {
            ...
            DDLogDebug(@"Command \"%@\" for application \"%@\"", command, application);
            ...
            if ([command isEqualToString:@"-"]) {
                ...
                @autoreleasepool {
                    ...
                    if (tmp) {
                        NSString *defaultBrowser = (__bridge NSString*)tmp;
            ...
            DDLogDebug(@"Key \"%@%@%@%@%@\" for application \"%@\"",
...
- (void)addMultitouchDevice:(NSTimer*)theTimer {
    ...
    DDLogVerbose(@"Adding device: %"PRIu64", try %d", newDeviceMultitouchID, attemptMT);
...
//        NSLog(@"Device's path in IOService plane = %s\n", pathName);
        ...
        if (deviceIDRef != NULL) {
            familyID = (int)[(__bridge NSString*)deviceIDRef integerValue];
//            NSLog(@"Device's family ID = %@ -> %d", deviceIDRef, familyID);
        ...
        if (deviceIDRef != NULL) {
            deviceID = [(__bridge NSString*)deviceIDRef integerValue];
...
static CGEventRef CGEventCallback(CGEventTapProxy proxy, CGEventType type, CGEventRef event, void *refcon) {
    if (type == kCGEventLeftMouseDown) {
        ...
        if (suppress) {
            DDLogDebug(@"Suppressed MouseDown with %d fingers d=%f t=%f", trackpadNFingers, twoFingersDistance, timeInterval);
            return NULL;
        } else {
            DDLogDebug(@"Did not suppress MouseDown with %d fingers d=%f t=%f", trackpadNFingers, twoFingersDistance, timeInterval);
        }
    } else {
        DDLogDebug(@"Did not suppress MouseUp with %d fingers d=%f", trackpadNFingers, twoFingersDistance);
    }
...
- (void)createEventTapTimer:(NSTimer *)timer {
    ...
    if (newEventTap == nil) {
        DDLogInfo(@"Could not create CGEventTap (try %d)", eventTapTries);
        eventTapTries++;
        if (eventTapTries < 360) {
            [NSTimer scheduledTimerWithTimeInterval:1.0 target:me selector:@selector(createEventTapTimer:) userInfo:nil repeats:NO];
        } else {
            DDLogWarn(@"Could not create CGEventTap after 5 minutes. Perhaps try removing Jitouch permissions and relaunching Jitouch.");
        }
    } else {
...
- (id)init {
    ...
    if (self = [super init]) {
        ...
        eventTap = [me createEventTap];
        if (eventTap == nil) {
            DDLogWarn(@"Could not create CGEventTap. Allow Jitouch in System Preferences -> Privacy -> Accessibility.");
            recreatingEventTap = TRUE;
            eventTapTries = 0;
            [NSTimer scheduledTimerWithTimeInterval:1.0 target:me selector:@selector(createEventTapTimer:) userInfo:nil repeats:NO];
        }
